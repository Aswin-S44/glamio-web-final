...
const EditServiceScreen = ({ navigation, route }) => {
  ...
  const [servicePrice, setServicePrice] = useState(
    String(initialService.servicePrice) || '',
  );
  const [imageUri, setImageUri] = useState(initialService.imageUrl || null);
  ...
  useEffect(() => {
    if (
      ...
      !predefinedCategories.includes(initialService.category)
    ) {
      setPredefinedCategories(prev => [initialService.category, ...prev]);
    }
  }, [initialService.category, predefinedCategories]);
  ...
  const handleUpdateService = async () => {
    ...
    try {
      ...
      const result = await updateService(initialService.id, serviceData);
      if (result) {
