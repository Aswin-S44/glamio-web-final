...
export default function EditProfileScreen() {
  ...
  // New state for Google Business Images toggle
    ...
    (async () => {
      try {
        ...
        if (data?.shop) {
          ...
          // If gallery images exist, set toggle to false (user has custom images)
          // Otherwise default to true (use Google Business Images)
          setUseGoogleImages(galleryImages.length === 0);
  ...
  const handleGalleryImages = async (e) => {
    ...
    if (remaining <= 0) {
      Swal.fire({
        icon: "warning",
        title: "Limit reached",
        text: `You can upload up to ${MAX_GALLERY} gallery photos.`,
        confirmButtonColor: "#c2185b",
      });
      return;
    }
    ...
    if (files.length > remaining) {
      Swal.fire({
        icon: "warning",
        title: "Photo limit",
        text: `You can upload up to ${MAX_GALLERY} gallery photos. Only the first ${remaining} of the ${files.length} selected were added.`,
        confirmButtonColor: "#c2185b",
      });
    }
    ...
    if (oversized.length) {
      Swal.fire({
        ...
        text: `${oversized.length} file(s) exceed 5 MB and were skipped.`,
        confirmButtonColor: "#c2185b",
      });
    }
    ...
    try {
      ...
      setSaved(false);
      // If user adds custom images, turn off Google Images toggle
      if (useGoogleImages) {
        setUseGoogleImages(false);
      }
    } catch {
      Swal.fire({
        icon: "error",
        title: "Upload failed",
        text: "Some images could not be read.",
        confirmButtonColor: "#c2185b",
      });
    }
  };
  ...
  /* ── Toggle Google Business Images ── */
  const handleToggleGoogleImages = () => {
    const newValue = !useGoogleImages;
    setUseGoogleImages(newValue);

    if (newValue) {
      // If turning ON Google Images, clear custom gallery images
      setForm((p) => ({ ...p, galleryImages: [] }));
      // Show a confirmation message
      Swal.fire({
        icon: "info",
        title: "Using Google Business Images",
        text: "Your shop gallery will use images from Google Business. Custom images will be removed.",
        confirmButtonColor: "#c2185b",
        timer: 3000,
        showConfirmButton: true,
      });
    } else {
      // If turning OFF, keep current images but allow user to add custom
      Swal.fire({
        icon: "info",
        title: "Custom Gallery Enabled",
        text: "You can now upload your own gallery images.",
        confirmButtonColor: "#c2185b",
        timer: 2000,
        showConfirmButton: true,
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Prepare gallery images based on toggle
      let galleryImagesToSend = form.galleryImages;

      // If useGoogleImages is true, send empty array (use Google Business Images)
      // If false, send custom gallery images
      if (useGoogleImages) {
        galleryImagesToSend = [];
      }
      ...
      const body = {
        shop: {
          ...
          // Add flag to indicate using Google Business Images
          useGoogleBusinessImages: useGoogleImages,
        },
    ...
    <div className="ep-root">
      ...
      <form className="ep-body" onSubmit={handleSubmit} noValidate>
        ...
        <div className="ep-main">
          ...
          <div className="ep-card">
            ...
            <div className="ep-grid">
              ...
              >
                ...
                <span
                  className={`ep-char ${
                    form.about.length > MAX_ABOUT ? "over" : ""
                  }`}
                >
          ...
          {/* Gallery images with Google Business toggle */}
          <div className="ep-card">
            <div className="ep-card-header-with-toggle">
              <div className="ep-card-title">
                <Images size={16} /> Shop Gallery
                <span className="ep-gallery-count">
                  {useGoogleImages
                    ? "Google Images"
            ...
            <p className="ep-gallery-hint">Add photos of your shop, interior, and work to attract more customers. You can upload up to {MAX_GALLERY} photos (max 5 MB each).</p>
              ...
              {/* <div className="ep-toggle-container">
                <button
                  ...
                  onClick={handleToggleGoogleImages}
                  title={
                    useGoogleImages
                      ? "Click to use custom images"
                      : "Click to use Google Business images"
                  }
                >
                  {useGoogleImages ? (
                    <ToggleRight size={24} className="ep-toggle-on" />
                  ) : (
                    <ToggleLeft size={24} className="ep-toggle-off" />
                  )}
                  <span className="ep-toggle-label">
                    Use Google Business Images
                  </span>
                </button>
              </div> */}
              <div className="ep-toggle-container">
                <button
                  ...
                  className={`ep-toggle-switch ${
                    useGoogleImages ? "active" : ""
                  }`}
                  onClick={handleToggleGoogleImages}
                >
                  <div className="ep-toggle-track">
            ...
            {useGoogleImages ? (
              // Show Google Images info
              <div className="ep-google-images-info">
                <div className="ep-google-images-banner">
                  <div className="ep-google-images-content">
                    <h4>Using Google Business Images</h4>
                    <p>
                      Your shop gallery will display images from your Google
                      Business profile automatically.
                    </p>
                    <div className="ep-google-images-features">
                      <span>
                        <CheckCircle size={14} /> Auto-sync with Google
                      </span>
                      <span>
                        <CheckCircle size={14} /> Always up-to-date
                      </span>
                      <span>
                        <CheckCircle size={14} /> No manual upload needed
                      </span>
                    </div>
                  </div>
                </div>
                <p className="ep-gallery-hint">
                  Toggle off to upload and manage custom gallery images.
                </p>
              </div>
            ) : (
              // Show custom gallery upload
              <>
                <p className="ep-gallery-hint">
                  Add photos of your shop, interior, and work to attract more
                  customers.
                </p>
        ...
        <aside className="ep-sidebar">
          ...
          <div className="ep-side-card ep-preview-card">
            ...
            <div className="ep-preview-body">
              <p className="ep-preview-name">
                {form.parlourName || "Your Parlour Name"}
              </p>
              {form.address && (
                <p className="ep-preview-addr">
          ...
          <div className="ep-side-card ep-tips">
            ...
            <ul>
              ...
              <li>
                Add gallery photos to showcase your work and attract clients
              </li>
              <li>Keep your description under 300 chars for best display</li>
              <li>
                Add a Google Review link to build trust with new customers
              </li>
            </ul>
          </div>
        </aside>
...
function TopBar({ loading, saved, onBack, onSave }) {
  return (
    <header className="ep-topbar">
      ...
      <button
        type="button"
        className={`ep-save-top ${saved ? "saved" : ""}`}
        onClick={onSave}
        disabled={loading}
      >
        {loading ? (
          <>
            <Loader2 size={15} className="ep-spin" /> Saving…
          </>
        ) : saved ? (
          <>
