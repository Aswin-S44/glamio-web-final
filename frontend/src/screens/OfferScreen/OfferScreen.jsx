...
function OfferScreen() {
  ...
  const handleSaveOffer = async (e) => {
    ...
    if (!image && !editingOffer) {
      setImageError("Service image is required to create an offer");
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.message && data.message.includes("Image upload failed")) {
  ...
  const SkeletonCard = () => (
    <div className="of-card skeleton-card">
      <div className="of-img-wrap skeleton-media">
      ...
      <div className="of-body">
        ...
        <div className="of-pricing-row">
          <div className="skeleton-text skeleton-old-price"></div>
          <div className="skeleton-text skeleton-new-price"></div>
        </div>
      </div>
  ...
  const ImageCarousel = ({ images }) => {
    ...
    return (
      <div
        className="of-carousel-container"
        onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}
      >
        <div className="of-carousel-slides">
          {images.map((img, index) => (
            <div
              key={index}
              className={`of-carousel-slide ${
        ...
        {hasMultipleImages && (
          <>
            <button
              className="of-carousel-btn of-carousel-prev"
              onClick={(e) => {
                e.stopPropagation();
                prevSlide();
              }}
            ...
            <button
              className="of-carousel-btn of-carousel-next"
              onClick={(e) => {
                e.stopPropagation();
                nextSlide();
              }}
            ...
            <div className="of-carousel-dots">
              {images.map((_, index) => (
                <button
                  key={index}
                  className={`of-carousel-dot ${
                    index === currentIndex ? "active" : ""
                  }`}
                  onClick={(e) => {
                    e.stopPropagation();
              ...
              {offers.map((offer) => (
                <div key={offer.id} className="of-card">
                  <div className="of-img-wrap">
                    <ImageCarousel images={offer?.service?.images || []} />
                  ...
                  <div className="of-body">
                    <span className="of-category-tag">
                      {allServices.find(
                        (service) => service.id === offer.service?.id
            ...
            <form onSubmit={handleSaveOffer} className="of-form">
              <div className="of-form-layout">
                <div className="of-form-side">
                  ...
                  {imageError && (
                    <div className="of-error-message">{imageError}</div>
                  )}
                  {!image && !editingOffer && !imageError && (
                    <div className="of-hint-text">Image is required *</div>
                  )}
                </div>
                ...
                <div className="of-form-main">
                  ...
                  <div className="of-row">
                    <div className="of-input-group">
                      <label>
                        <IndianRupee size={14} /> Regular Price
                      </label>
                      <input
                        type="number"
                        value={regularPrice}
