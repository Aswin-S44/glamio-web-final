...
export default function SelectSlotScreen() {
  ...
  useEffect(() => {
    fetch(`${BASE_URL}/customer/shop/${id}`)
      .then((res) => (res.ok ? res.json() : null))
      .then((data) => {
        if (Array.isArray(data?.offers)) setShopOffers(data.offers);
      })
      .catch(() => {});
  }, [id]);
  ...
  useEffect(() => {
    ...
    setSelectedServices((prev) => {
      ...
      const next = prev.map((service) => {
        if (service.hasOffer) return service;
        const offer = shopOffers.find((o) => o.serviceId === service.id);
        if (!offer) return service;
  ...
  const isSlotBookable = (slot) => slot.isAvailable;

  const tileClassName = ({ date, view }) => {
    if (view !== "month") return null;
    const key = format(date, "yyyy-MM-dd");
    return slotsByDate[key]?.some(isSlotBookable)
      ? "has-slots-indicator"
      : null;
  };
          ...
          {["Select Specialist", "Choose Date & Time", "Confirm"].map(
            (label, i) => (
              <React.Fragment key={i}>
                ...
                {i < 2 && (
                  <div
                    className={`ss-step-line ${step > i + 1 ? "filled" : ""}`}
                  />
                )}
              </React.Fragment>
            )
      ...
      <div className="ss-body">
          ...
          <section className="ss-section">
            <div className="ss-section-head">
              ...
              <div>
                ...
                <p>How many people will be availing this service?</p>
              </div>
            </div>

            <div className="ss-people-selector">
              <button
                className="ss-people-btn"
                onClick={() => handlePeopleChange(false)}
                disabled={numberOfPeople <= 1}
              >
                <Minus size={18} />
              </button>
              <div className="ss-people-count">
                <span className="ss-people-number">{numberOfPeople}</span>
                <span className="ss-people-label">
                        ...
                        {daySlots
                          ...
                          .map((slot) => (
                            <SlotBtn
                              ...
                              alreadyBooked={!slot.isAvailable}
                              onClick={() => {
        ...
        <aside className="ss-sidebar">
          <div className="ss-summary-card">
            ...
            <div className="ss-summ-block">
              <span className="ss-summ-label">People</span>
              <div className="ss-summ-people">
                <Users size={14} />
                <span>
              ...
              {expertData ? (
                <div className="ss-summ-expert">
                  ...
                  <div>
                    ...
                    <span>
                      {Array.isArray(expertData.specialist)
                        ? expertData.specialist.join(", ")
          ...
          <div className="ss-trust">
            <div className="ss-trust-item">Free cancellation up to 2 hrs</div>
...
function SlotBtn({ slot, selected, onClick, alreadyBooked }) {
  return (
    <button
    ...
    >
      ...
      {alreadyBooked ? (
        <span className="ss-slot-tag">Already booked</span>
      ) : (
